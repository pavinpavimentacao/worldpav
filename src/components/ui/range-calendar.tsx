...
const CalendarHeading = (props: React.HTMLAttributes<HTMLElement>) => {
  ...
  return (
    <header className="flex w-full items-center gap-1 px-1 pb-4" {...props}>
      ...
      <AriaHeading className="grow text-center text-sm font-semibold text-foreground" />
      <AriaButton
        slot="next"
        className={cn(
          buttonVariants({ variant: "outline", size: "icon" }),
...
const CalendarGrid = ({ className, ...props }: AriaCalendarGridProps) => (
  <AriaCalendarGrid
    className={cn(
      "border-separate border-spacing-x-0 border-spacing-y-1 rounded-lg border border-border bg-card shadow-sm",
      className
    )}
    {...props}
  />
...
const CalendarHeaderCell = ({
  ...
  <AriaCalendarHeaderCell
    className={cn(
      "w-9 rounded-md text-[0.8rem] font-medium text-muted-foreground py-2",
      className
    )}
    {...props}
  />
...
const CalendarGridBody = ({
  ...
  <AriaCalendarGridBody className={cn("[&>tr>td]:p-1", className)} {...props} />
)
