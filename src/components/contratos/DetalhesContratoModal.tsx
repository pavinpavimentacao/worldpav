...
export function DetalhesContratoModal({ contrato, onClose, onEdit, onDelete }: DetalhesContratoModalProps) {
  return (
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white z-10">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-gray-900">{contrato.name}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-gray-900 capitalize">{contrato.type}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-500 mb-1">Valor</label>
              <p className="text-gray-900 font-semibold">{currency(contrato.value)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-gray-900">{format(new Date(contrato.start_date), 'dd/MM/yyyy')}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-500 mb-1">Data de Fim</label>
              <p className="text-gray-900">{contrato.end_date ? format(new Date(contrato.end_date), 'dd/MM/yyyy') : 'Indefinido'}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-gray-900">{contrato.cliente_nome || 'Cliente não informado'}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-500 mb-1">Obra</label>
              <p className="text-gray-900">{contrato.obra_nome || 'Não vinculada a obra específica'}</p>
            </div>
          </div>
          ...
          {contrato.observations && (
            <div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <p className="text-gray-700 whitespace-pre-wrap">{contrato.observations}</p>
              </div>
            </div>
          )}
          ...
          {contrato.file_path && (
            <div>
              ...
              <div className="flex items-center gap-3">
                <span className="text-gray-700">📄 {contrato.file_name || 'Documento anexado'}</span>
          ...
          <div className="pt-4 border-t border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="text-gray-600">{format(new Date(contrato.created_at), 'dd/MM/yyyy HH:mm')}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-500 mb-1">Última atualização</label>
                <p className="text-gray-600">{format(new Date(contrato.updated_at), 'dd/MM/yyyy HH:mm')}</p>
              </div>
            </div>
          </div>
        </div>
