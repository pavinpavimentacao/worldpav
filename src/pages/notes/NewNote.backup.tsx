...
export const NewNote: React.FC = () => {
  ...
  const onSubmit = async (data: NoteFormData) => {
    try {
      ...
      // Criar um report temporário para a nota
      const { data: tempReport, error: reportError } = await supabase
        .from('reports')
        .insert({
          pump_id: null,
          client_id: null,
          ...
          hours_worked: 8,
          status: 'NOTA_EMITIDA',
          observations: data.descricao || 'Nota fiscal gerada manualmente'
        })
        .select('id')
        .single();

      if (reportError) {
        throw new Error('Erro ao criar report temporário: ' + reportError.message);
      }

      // Preparar dados para a API de geração de notas
      const noteData = {
        report_id: tempReport.id,
      ...
      // Chamar a API de geração de notas
      const result = await generateInvoiceNote(noteData);

      if (result.ok) {
        // Sucesso! Redirecionar para lista com mensagem de sucesso
        navigate('/notes', { 
          state: { 
            message: 'Nota fiscal gerada com sucesso!',
            noteNumber: result.note.nf_number 
          } 
        });
      } else {
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="flex justify-end space-x-3 pt-6 border-t">
                  ...
                  <Button
                    ...
                    disabled={isSubmitting}
                  >
                    {isSubmitting ? 'Gerando Nota...' : 'Gerar Nota Fiscal'}
                  </Button>
                </div>
              </form>
            </div>
