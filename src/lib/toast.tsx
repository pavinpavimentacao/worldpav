...
function ToastItem({ toast, onRemove }: { toast: Toast, onRemove: (id: string) => void }) {
  ...
  return (
    <div
      className={clsx(
        'w-full bg-white shadow-xl rounded-lg pointer-events-auto overflow-hidden transform transition-all duration-300 ease-in-out',
    ...
    >
      <div className="p-5">
        <div className="flex items-start">
          <div className={clsx('flex-shrink-0 mr-3', iconStyles[toast.type])}>
            {icons[toast.type]}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold leading-5 text-gray-900">{toast.message}</p>
          </div>
